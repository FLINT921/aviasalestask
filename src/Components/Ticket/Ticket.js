...
const Ticket = ({ tickets, loading, error, getSearchId, filterTransfers, filterFlight }) => {
  ...
  const getTickets = (tickets, filterTransfers, filterFlight) => {
    ...
    if (filterTransfers.withoutTransfers) {
      const withoutTransfersTickets = tickets.filter(
    ...
    if (filterTransfers.oneTransfers) {
      const oneTransfers = tickets.filter(
    ...
    if (filterTransfers.twoTransfers) {
      const twoTransfers = tickets.filter(
    ...
    if (filterTransfers.threeTransfers) {
      const threeTransfers = tickets.filter(
    ...
    const uniqueTickets = Array.from(new Set(filteredTickets.map((ticket) => JSON.stringify(ticket)))).map((ticket) =>
      JSON.parse(ticket),
    );
    ...
    if (filterFlight.fastestFlight) {
      sortedTickets = uniqueTickets.sort(
  ...
  return (
    <div className={classes.Tickets}>
      ...
      {getTickets(tickets, filterTransfers, filterFlight).length === 0 && !loading && !error && (
        <span>Рейсов, подходящих под заданные фильтры, не найдено</span>
      )}
      {getTickets(tickets, filterTransfers, filterFlight).length === 0 && error && (
        <span>Самолеты сегодня не летают! Попробуйте перезагрузить страницу..</span>
      )}
      {getTickets(tickets, filterTransfers, filterFlight).map((ticket, index) => (
        <div key={index} className={classes.Ticket}>
          <div className={classes.header}>
            <p className={classes.price}>
              {`${ticket.price}`.slice(0, `${ticket.price}`.length - 3)}{' '}
              {`${ticket.price}`.slice(`${ticket.price}`.length - 3, `${ticket.price}`.length)} Р
            </p>
          ...
          {ticket.segments.map((segment, segmentIndex) => (
            <div
              key={segmentIndex}
              className={classNames(classes.flight, segmentIndex === 0 ? classes.flightOne : classes.flightTwo)}
            >
              <div className={classes.rout}>
                ...
                <p>
                  {new Date(segment.date).toLocaleTimeString().slice(0, 5)} -{' '}
                  {new Date(new Date(segment.date).getTime() + segment.duration * 60000)
                    .toLocaleTimeString()
                    .slice(0, 5)}
                </p>
              </div>
